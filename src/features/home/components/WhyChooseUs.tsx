  ...
  return (
    <SectionWrapper className="space-y-10" id="benefits">
      ...
      <motion.div
        ...
        whileInView={{ opacity: 1, y: 0 }}
      >
        {BENIFITS_BADGES.map((benefit: string) => (
          <Badge key={benefit} className="text-sm" variant="secondary">
            {benefit}
          </Badge>
        ))}
      </motion.div>
      ...
      <motion.div
        ...
        whileInView={{ opacity: 1, y: 0 }}
      >
        {BENIFITS.map(({ icon, title, description }: BenifitsProps, index) => (
          <motion.div
            key={title}
            ...
            whileInView={{ opacity: 1, y: 0 }}
          >
            <Card className="flex min-h-[280px] flex-col justify-center border bg-muted/50 text-center">
